...
def draw(event):
    ...
    if is_drawQuestionMark:
        ...
        undo_stack.append(('line', line, b4_x, b4_y, rn_x, rn_y, draw_color, line_width))
        b4_x, b4_y = rn_x, rn_y  # Update b4_x and b4_y after drawing the line
...
def add_text_entry(event):
    ...
    if text_tool_active:
        ...
        if text:
            ...
            undo_stack.append(('text', text_id, event.x, event.y, text, draw_color, line_width))
            redo_stack.clear()  # Clear redo stack when a new action is taken
...
def undo_action():
    if undo_stack:
        ...
        canvas.delete(last_action[1])  # Delete the last item from the canvas
...
def redo_action():
    if redo_stack:
        ...
        if action_type == 'line':
            ...
            line = canvas.create_line(x1, y1, x2, y2, fill=color, width=width, capstyle=TK.ROUND, joinstyle="round", smooth=True)
            undo_stack.append(('line', line, x1, y1, x2, y2, color, width))
        
        elif action_type == 'text':
            _, _, x, y, text, color, width = last_undone_action
            text_id = canvas.create_text(x, y, text=text, fill=color, font=("Arial", width * 2))
            undo_stack.append(('text', text_id, x, y, text, color, width))
